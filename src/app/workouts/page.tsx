...
function WorkoutListContent() {
  const { workoutPlans, categories, loading: workoutsLoading } = useWorkouts();
  ...
  }, [workoutPlans, selectedCategory, searchTerm]);
    ...
    <div className="space-y-8 animate-fadeIn">
      ...
      {filteredWorkoutPlans.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredWorkoutPlans.map((plan: WorkoutPlan) => (
            <WorkoutCard key={plan.id} plan={plan} />
          ))}
        </div>
      ) : (
        <div className="text-center py-12">
          <p className="text-xl text-muted-foreground">No workout plans found matching your criteria.</p>
